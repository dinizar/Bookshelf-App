...
for (i = 0; i < acc.length; i++) {
  ...
  document.getElementById("inputBookIsComplete").addEventListener("change", function () {
    ...
    if (isComplete) {
      document.getElementById("spanForm").textContent="Sudah dibaca";
    } else {
      document.getElementById("spanForm").textContent="Belum dibaca";
    }
  });
  ...
  formAddingBook.addEventListener("submit", function (_event) {
    ...
    if (title !== '' && author !== '' && year !== '') {
      const panelForm = document.getElementsByClassName("panel add");
      panelForm[0].style.display = "none";
    }
    ...
    if (idTemp !== "") {
      ...
      for (let [index] of bookData.entries()) {
  ...
  function RenderBookSearch(bookData) {
    if (bookData === null) {
      return;
    }
    ...
    for (let book of bookData) {
      ...
      bookItem.innerHTML += isComplete ? "<p class='status-search-sudah'>Sudah Dibaca</p>" : "<p class='status-search-belum'>Belum dibaca</P>";
    ...
    // open result
    const panelForm = document.getElementsByClassName("panel result");
    panelForm[0].style.display = "block";
  }
  ...
  function isCompleteBookHandler(itemElement) {
    ...
    for (const [index] of bookData.entries()) {
  ...
  searchBook.addEventListener("submit", function (event) {
    ...
    if (title === null) {
      RenderBookSearch(bookData);
      return;
    }
    const bookList = SearchBookList(title);
    RenderBookSearch(bookList);
  });
